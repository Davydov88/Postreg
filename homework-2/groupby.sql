...
SELECT order_id, customer_id, weight, ship_country FROM orders WHERE ship_country LIKE 'P%' ORDER BY weight DESC LIMIT 10;

-- 3. фамилию и телефон сотрудников, у которых в данных отсутствует регион (см таблицу employees)
SELECT last_name, phone FROM employees WHERE region IS NULL;
...
SELECT ship_country, SUM(weight) AS total_weight FROM orders WHERE ship_region IS NOT NULL GROUP BY ship_country HAVING SUM(weight) > 2750 ORDER BY total_weight DESC;
